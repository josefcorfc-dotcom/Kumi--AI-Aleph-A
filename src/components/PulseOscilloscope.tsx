...
import { Play, Pause, Activity, Zap, Eye, RotateCcw, Download, BarChart2 } from 'lucide-react';
import { SimulationParams, PulseDataPoint } from '../types';
import { createBioDigitalSimulator } from '../utils/neuro';
import { audioSynth } from '../utils/audio';
import { computeHarmonicSpectrum } from '../utils/fft';
...
export const PulseOscilloscope: React.FC<PulseOscilloscopeProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'waveform' | 'fft'>('waveform');
  ...
  useEffect(() => {
    ...
    const renderLoop = (now: number) => {
      ...
      if (viewMode === 'fft') {
        // Render FFT Frequency Domain Harmonic Spectrum
        ctx.fillStyle = '#38bdf8';
        ctx.font = '11px monospace';
        ctx.fillText('FREQUENCY DOMAIN: FFT Harmonic Spectrum (Node SQ-3000_G6 - Master Clock 97.50 GHz)', 14, 22);

        const voltages = dataBufferRef.current.map(p => p.unifiedNeuroBin);
        const spectrum = computeHarmonicSpectrum(voltages, 1000);

        if (spectrum.length > 0) {
          const maxAmp = Math.max(...spectrum.map(b => b.amplitude), 0.1);
          const barWidth = width / spectrum.length;

          spectrum.forEach((bin, idx) => {
            const x = idx * barWidth;
            const barHeight = (bin.amplitude / maxAmp) * (height - 80);
            ...
            ctx.fillStyle = grad;
            ctx.fillRect(x + 2, y, Math.max(barWidth - 4, 2), barHeight);

            if (idx % 8 === 0) {
              ctx.fillStyle = '#94a3b8';
              ctx.font = '9px monospace';
              ctx.fillText(`${bin.frequency.toFixed(1)} Hz`, x, height - 12);
            }
          });
        }
      } else {
        ...
        if (showNeuro) {
          ...
          for (let i = 0; i < pts.length; i++) {
            ...
            const normV = (v - params.restingPotentialMv) / 100;
  ...
  }, [isPlaying, params, historyLength, showNeuro, showBin, showUnified, viewMode, onSpikeOccurred]);
    ...
    <div id="oscilloscope-container" className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl flex flex-col gap-4 relative overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white/5 backdrop-blur-md px-4 py-3 rounded-2xl border border-white/10 shadow-inner">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-bold text-white uppercase tracking-wider">
              Real-Time Bio-Digital Oscilloscope & FFT Spectrum
            </h2>
            <span className="text-[10px] text-slate-400 font-mono tracking-widest uppercase">SQ-3000_G6 • 60 FPS Analyzer</span>
          </div>
        </div>
        ...
        <div className="flex items-center flex-wrap gap-2 text-xs">
          ...
          {/* View Mode Toggle (Waveform vs FFT) */}
          <button
            id="view-mode-toggle-btn"
            onClick={() => setViewMode(viewMode === 'waveform' ? 'fft' : 'waveform')}
            className={`flex items-center gap-1.5 px-3.5 py-1 rounded-full border font-mono text-[11px] font-semibold transition backdrop-blur-md ${
              viewMode === 'fft'
          ...
          >
            ...
            <span>{viewMode === 'waveform' ? 'FFT Spectrum' : 'Waveform'}</span>
          </button>
